...
class TestCLI:
    ...
    def test_optimize_with_objective(
        self, mock_log, mock_config, mock_orch, tmp_path: Path
    ):
        ...
        result = runner.invoke(app, ["optimize", str(f), "--objective", "price"])
        assert result.exit_code == 0
        instance.run_optimize.assert_called_once()
        call_kwargs = instance.run_optimize.call_args.kwargs
        assert call_kwargs.get("objective") == "price"
